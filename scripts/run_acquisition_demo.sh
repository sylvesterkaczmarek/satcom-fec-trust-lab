...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'EOF'
Usage: scripts/run_acquisition_demo.sh [iq_path] [metadata_path]

Runs the portable scalar acquisition search. When metadata_path is omitted,
the tool reads a JSON sidecar with the same basename as the IQ fixture.
EOF
  exit 0
fi
...
"${BIN_PATH}" --iq "${IQ_PATH}" --metadata "${METADATA_PATH}"
