...
#if defined(SATCOMFEC_FEC_STREAMING_VECTOR_COMPILED) && \
...
#include <arm_sme.h>
#endif
#define SATCOMFEC_FEC_HAS_STREAMING_VECTOR 1
#else
#define SATCOMFEC_FEC_HAS_STREAMING_VECTOR 0
#endif

namespace satcomfec {

#if SATCOMFEC_FEC_HAS_STREAMING_VECTOR
namespace {
...
// Historical experiment: this uses SVE-style operations in locally streaming
// mode. It does not use ZA or an SME2-specific multi-vector instruction.
__arm_locally_streaming bool prepare_branch_metrics_streaming_vector_kernel(
    const SoftBitBuffer& soft_in,
    BranchMetricTables& tables) {
    if (soft_in.empty() || (soft_in.size() % 2) != 0) {
        log_error(
            "prepare_branch_metrics_streaming_vector: expected an even number "
            "of soft decisions");
        return false;
    }
    ...
    while (symbol_index < symbol_count) {
        ...
        const svint16_t soft0 = svunpklo_s16(svget2_s8(pair, 0));
        const svint16_t soft1 = svunpklo_s16(svget2_s8(pair, 1));

        const svint16_t sum = svadd_s16_x(pg16, soft0, soft1);
        const svint16_t neg_sum = svsub_s16_x(pg16, svdup_s16(0), sum);
        const svint16_t soft1_minus_soft0 =
            svsub_s16_x(pg16, soft1, soft0);
        const svint16_t soft0_minus_soft1 =
            svsub_s16_x(pg16, soft0, soft1);
...
bool prepare_branch_metrics_streaming_vector(const SoftBitBuffer& soft_in,
                                             BranchMetricTables& tables) {
#if SATCOMFEC_FEC_HAS_STREAMING_VECTOR
    return prepare_branch_metrics_streaming_vector_kernel(soft_in, tables);
#else
...
bool branch_metrics_streaming_vector_kernel_compiled() {
    return SATCOMFEC_FEC_HAS_STREAMING_VECTOR != 0;
}
